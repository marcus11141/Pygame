...
class Platform:
    ...
    def draw(self, display_area, sprites=None):
        ...
        # === 步驟12: 優先使用圖片，找不到則用方塊 ===
        if sprites:
            # 紅色平台用break_platform，其餘用std_platform
            if self.special:
...
def load_doodle_sprites():
    """
    載入image資料夾中的src.png圖片，並根據sprite_data字典裁切出平台、彈簧等物件圖片，
    另外載入玩家的左右跳躍/下落圖片，全部存到sprites字典並回傳。
    """
    # 設定圖片資料夾路徑為目前執行檔案的資料夾
